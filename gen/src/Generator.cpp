...
{
// A generated file travels on its own: the headers are installed one by one, so
// whatever a reader has to know has to be in the file itself. Class names,
// element descriptions and enum comments here are rendered from the
// Open-XML-SDK metadata snapshot under data/, which is MIT-licensed by the .NET
// Foundation, and MIT asks for that notice in every substantial portion. The
// banner therefore names it and says where the text lives, rather than leaving
// the file to read as though it were wholly this project's own.
