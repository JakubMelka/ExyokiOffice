...
Size IndentOf(std::string_view line)
{
    Size indent = 0;
...
std::string_view StripIndent(std::string_view line, Size count)
{
    Size columns = 0;
    Size offset = 0;
...
bool IsThematicBreakLine(std::string_view line)
{
...
bool TryParseListMarker(std::string_view line, ListMarker& marker)
{
...
bool HasUnescapedPipe(std::string_view line)
{
...
bool IsTableDelimiterLine(std::string_view line)
{
    const auto trimmed = TrimView(line);
...
bool TryParseFootnoteDefinitionStart(std::string_view line, std::string& label, std::string_view& content)
{
    const auto trimmed = TrimView(line);
...
bool TryParseAtxHeading(std::string_view line, int& level, std::string_view& content)
{
    const auto trimmed = TrimView(line);
    Size hashes = 0;
...
bool IsFenceLine(std::string_view line, std::string_view& info)
{
    const auto trimmed = TrimView(line);
    if (!trimmed.starts_with("```"))
    {
...
MarkdownDocumentAst ParseMarkdown(std::string_view text, std::vector<ToolDiagnostic>& diagnostics)
{
    ...
    document.Blocks = BlockParser::ParseLines(lines, 0, lines.size(), diagnostics);
    return document;
}

std::string RenderMarkdown(const MarkdownDocumentAst& document)
{
    return MarkdownWriter::Render(document);
}
