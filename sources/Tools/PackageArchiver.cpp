...
void AddDiagnostic(std::vector<ToolDiagnostic>& diagnostics, ToolSeverity severity, std::string message,
                   std::string context = {})
{
    diagnostics.push_back(ToolDiagnostic{severity, std::move(message), std::move(context)});
}

bool IsReservedWindowsName(std::string_view baseName)
{
...
std::string PercentEncodeByte(unsigned char value)
{
 ...
 * edited.
 */
bool IsForbiddenSegmentByte(unsigned char ch)
{
...
bool IsSafeWindowsSegment(std::string_view segment)
{
...
std::string EscapeWindowsSegment(std::string_view segment)
{
    std::string escaped;
 ...
 * nothing else.
 */
bool HasTraversalSegment(const std::vector<std::string>& segments)
{
    return std::find(segments.begin(), segments.end(), "..") != segments.end();
}

std::vector<std::string> SplitEntryPath(std::string_view entryName)
{
 ...
 */
std::pair<std::filesystem::path, bool> MapEntryToDiskPath(std::string_view entryName)
{
...
bool LooksLikeXml(std::string_view entryName, const std::vector<Byte>& bytes)
{
...
std::vector<Byte> PrettyPrintXmlBytes(const std::vector<Byte>& bytes, bool& ok)
{
...
std::string PathToPortableString(const std::filesystem::path& path)
{
...
const std::vector<std::pair<std::string, std::string>>& DefaultContentTypeTable()
{
                    ...
                    const UnpackOptions& options)
{
    ...
    {
        AddDiagnostic(result.Diagnostics, ToolSeverity::Error, "Output directory is not empty",
                      outDir.string());
        return result;
    }
    ...
    if (!archive)
    {
        AddDiagnostic(result.Diagnostics, ToolSeverity::Error,
    ...
    if (total < 0)
    {
        AddDiagnostic(result.Diagnostics, ToolSeverity::Error, "ZIP entry table cannot be read",
                      packagePath.string());
        zip_close(archive);
        return result;
    }
    ...
    {
        AddDiagnostic(result.Diagnostics, ToolSeverity::Error, std::string(reason), packagePath.string());
        zip_close(archive);
        return result;
    }
    ...
    for (ssize_t index = 0; index < total; ++index)
    {
        ...
        const auto segments = SplitEntryPath(entryName);
        if (HasTraversalSegment(segments))
        {
            AddDiagnostic(result.Diagnostics, ToolSeverity::Error, "Rejected entry with path traversal", entryName);
            zip_entry_close(archive);
            continue;
        }
        ...
        {
            AddDiagnostic(result.Diagnostics, ToolSeverity::Error, std::string(reason), entryName);
            zip_entry_close(archive);
            zip_close(archive);
            return result;
        }
        ...
        {
            AddDiagnostic(result.Diagnostics, ToolSeverity::Error, "Failed to read entry", entryName);
            continue;
        }
        ...
        auto [diskPath, wasEscaped] = MapEntryToDiskPath(entryName);
        if (wasEscaped)
        {
            renames.push_back(RenameEntry{entryName, PathToPortableString(diskPath)});
        }

        if (options.PrettyPrintXml && LooksLikeXml(entryName, bytes))
        {
            bool prettyOk = false;
            bytes = PrettyPrintXmlBytes(bytes, prettyOk);
        ...
        if (!file)
        {
            AddDiagnostic(result.Diagnostics, ToolSeverity::Error, "Failed to write extracted entry",
                          fullPath.string());
            continue;
        }
    ...
    if (!renames.empty() || anyPrettyPrinted)
    {
        ...
        {
            manifest << "  <rename from=\"" << XmlEscape(rename.OriginalName) << "\" to=\""
                     << XmlEscape(rename.DiskRelativePath) << "\"/>\n";
        }
 ...
 * not.
 */
bool IsRestorableEntryName(std::string_view entryName)
{
    ...
    const auto segments = SplitEntryPath(entryName);
    if (segments.empty() || HasTraversalSegment(segments))
    {
        return false;
    }
...
std::vector<std::pair<std::string, std::string>> LoadRenameManifest(const std::filesystem::path& inDir,
                                                                    std::vector<ToolDiagnostic>& diagnostics)
{
    ...
    for (auto node = root.child("rename"); node; node = node.next_sibling("rename"))
    {
        ...
        {
            AddDiagnostic(diagnostics, ToolSeverity::Warning,
...
std::vector<Byte> BuildDefaultContentTypesXml(const std::vector<std::filesystem::path>& relativeFiles)
{
    ...
    const auto& table = DefaultContentTypeTable();
                ...
                const PackOptions& options)
{
    ...
    {
        AddDiagnostic(result.Diagnostics, ToolSeverity::Error, "Input directory does not exist", inDir.string());
        return result;
    }
    ...
    if (regenerate)
    {
        const auto bytes = BuildDefaultContentTypesXml(relativeFiles);
    ...
    for (const auto& relative : relativeFiles)
    {
        ...
        {
            AddDiagnostic(result.Diagnostics, ToolSeverity::Error, "Failed to open ZIP entry for writing",
                          entryName);
            continue;
        }
        if (zip_entry_write(archive, bytes.data(), bytes.size()) < 0)
        {
            AddDiagnostic(result.Diagnostics, ToolSeverity::Error, "Failed to write ZIP entry", entryName);
        }
        else
        {
            ++result.EntryCount;
