...
ValidationIssue MakeOpcIssue(ValidationErrorId id,
                             std::string message,
...
ValidationIssue MakePackageSchematronIssue(std::string message,
...
ValidationIssue MakePackageSchematronElementIssue(std::string message,
                                                  const OpenXmlPackagePart& part,
                                                  const OpenXMLElement& element,
...
const OpenXmlRelationship* FindRelationshipById(const OpenXmlPartContainer& container, std::string_view id)
{
...
std::string NormalizeSchematronUniqueValue(std::string_view value, bool caseInsensitive)
{
...
void VisitOpenXmlTree(const std::shared_ptr<OpenXMLElement>& element, TCallback&& callback)
{
    if (!element)
    {
        return;
...
bool MatchesElementPath(const OpenXMLElement& element, std::span<const OpenXmlQualifiedName> path)
{
...
void ResolvePartSelectorPath(const std::vector<std::shared_ptr<OpenXmlPackagePart>>& candidates,
...
std::vector<std::shared_ptr<OpenXmlPackagePart>> ResolvePackageSchematronPartSelector(
...
bool TryParseSchematronNumber(std::string_view text, Real& value)
{
...
bool EvaluateSchematronComparison(Real lhs,
...
std::string_view PackageSchematronComparisonOperatorText(
...
void CollectPackageSchematronAttributeValues(const OpenXmlPackagePart& targetPart,
...
Size CountPackageSchematronElements(const OpenXmlPackagePart& targetPart,
                                    std::span<const OpenXmlQualifiedName> elementPath)
{
    if (!targetPart.IsXmlPart())
    {
...
void ValidatePackageSchematronElement(const OpenXmlPackagePart& part,
                                      const OpenXMLElement& element,
                                      DiagnosticSink& sink)
{
...
void CollectPackageSchematronUniqueValue(const OpenXmlPackagePart& part,
                                         const OpenXMLElement& element,
                                         std::span<const OpenXmlQualifiedName> elementPath,
                                         const OpenXmlQualifiedName& attributeName,
...
void ValidatePackageSchematronUniqueValues(const OpenXmlPackagePart& part,
...
void ValidatePackageSchematronAncestorUniqueValues(const OpenXmlPackagePart& part,
                                                   const std::shared_ptr<OpenXMLElement>& root,
                                                   DiagnosticSink& sink)
{
...
void ValidatePackageSchematronPartReferences(const OpenXmlPackage& package,
                                             const OpenXmlPackagePart& part,
                                             const std::shared_ptr<OpenXMLElement>& root,
                                             DiagnosticSink& sink)
{
...
void ValidatePackageSchematronPartCounts(const OpenXmlPackage& package,
                                         const OpenXmlPackagePart& part,
                                         const std::shared_ptr<OpenXMLElement>& root,
                                         DiagnosticSink& sink)
{
...
void ValidatePackageSchematronTree(const OpenXmlPackagePart& part,
...
void ValidateContainerRelationships(const OpenXmlPackage& package,
...
ValidationIssue MakePackageIssue(ValidationErrorId id, std::string message, std::string_view partUri = {})
{
    ValidationIssue issue;
    issue.Severity = ValidationSeverity::Error;
    issue.Domain = ValidationDomain::Packaging;
...
bool HasValidDescriptorContentType(const OpenXmlPackagePart& part) noexcept
{
...
void ValidatePartSemantics(const OpenXmlPackage& package,
                           const OpenXmlPackagePart& part,
...
void ValidateMainPart(const OpenXmlPackage& package, DiagnosticSink& sink)
{
 ...
 * not a hot path.
 */
void ValidateXmlPartWellFormed(const OpenXmlPackagePart& part, DiagnosticSink& sink)
{
...
void ValidatePartTree(const OpenXmlPackage& package,
                      const OpenXmlPackagePart& part,
                      DiagnosticSink& sink,
 ...
 */
void ValidateConformanceClass(const OpenXmlPackage& package, DiagnosticSink& sink)
{
...
void ValidateDuplicatePartUris(const std::vector<std::string>& duplicatePartUris, DiagnosticSink& sink)
{
...
{
    ValidateDuplicatePartUris(package.m_impl->duplicatePartUris, sink);
    ValidateConformanceClass(package, sink);
    ValidateContainerRelationships(package, package, "/", sink);
    ValidateMainPart(package, sink);
    std::unordered_set<const OpenXmlPackagePart*> visited;
    for (const auto& part : package.Parts())
    {
        if (part)
        {
            ValidatePartTree(package, *part, sink, visited, domSettings_, validateDom_);
        }
    }
