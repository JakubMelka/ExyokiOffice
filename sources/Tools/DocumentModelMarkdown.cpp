...
Md::MarkdownInline MakeText(std::string text)
{
...
Md::MarkdownInline MakeWrapped(Md::MarkdownInline::Type kind, std::vector<Md::MarkdownInline> children)
{
    Md::MarkdownInline node;
    node.Kind = kind;
...
bool IsMonospaceFont(const std::string& font)
{
    return font == "Consolas" || font == "Courier New" || font == "Courier";
}

std::string InlinePlainText(const std::vector<Md::MarkdownInline>& inlines)
{
    std::string text;
...
const MediaReference* FindMedia(const DocumentModel& model, const std::string& mediaId)
{
...
Md::MarkdownDocumentAst ExcelToMarkdown(const ExcelWorkbookModel& workbook,
...
void MarkdownToExcel(const Md::MarkdownDocumentAst& ast, DocumentModel& model,
                     std::vector<ToolDiagnostic>& diagnostics)
{
...
std::string SerializeModelMarkdown(const DocumentModel& model, std::vector<ToolDiagnostic>& diagnostics)
{
    ...
    switch (model.Family)
    {
        case DocumentFamily::Word:
        {
            if (model.Word)
            {
                WordToMarkdown converter(model, diagnostics);
                ast = converter.Build();
            }
            break;
        }
        case DocumentFamily::Excel:
        {
            if (model.Excel)
            {
                ast = ExcelToMarkdown(*model.Excel, diagnostics);
            }
            break;
        }
        case DocumentFamily::PowerPoint:
        {
            if (model.PowerPoint)
            {
                PowerPointToMarkdown converter(model, diagnostics);
                ast = converter.Build();
            }
            break;
        }
                                 ...
                                 std::vector<ToolDiagnostic>& diagnostics)
{
    ...
    switch (targetFamily)
    {
        case DocumentFamily::Word:
        {
            ...
            model.Word.emplace();
            MarkdownToWord converter(model);
            converter.Build(ast);
            break;
        }
        case DocumentFamily::Excel:
        {
            ...
            MarkdownToExcel(ast, model, diagnostics);
            break;
        }
        case DocumentFamily::PowerPoint:
        {
            model.Family = DocumentFamily::PowerPoint;
            MarkdownToPowerPoint converter(model, diagnostics);
            converter.Build(ast);
            break;
        }
        case DocumentFamily::Unknown:
