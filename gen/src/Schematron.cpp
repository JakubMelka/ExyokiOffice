...
{
/// File-local helpers for reading and normalizing Schematron rules.
class SchematronHelper
{
public:
    static constexpr std::string_view kAttributeNamePattern = R"([A-Za-z_][A-Za-z0-9_.-]*(?::[A-Za-z_][A-Za-z0-9_.-]*)?)";

    static bool Match(const std::string& text, const std::regex& pattern, std::smatch& match)
    {
        return std::regex_match(text, match, pattern);
    }

    static std::vector<std::string> Captures(const std::smatch& match)
    {
    ...
    static bool IsAttributeName(std::string_view value)
    {
    ...
    static std::string StripSchematronLiteral(std::string value)
    {
    ...
    static bool TryExtractAllowedValues(const std::string& test, std::vector<std::string>& operands)
    {
     ...
     */
    static bool TryExtractMutuallyExclusiveAttributes(const std::string& test, std::vector<std::string>& operands)
    {
    ...
    static bool TryExtractForbiddenValues(const std::string& test, std::vector<std::string>& operands)
    {
        static const std::regex clause("^@(" + std::string(kAttributeNamePattern) + R"()\s*!=\s*([^ ]+)$)");
    ...
    static bool TryExtractParenthesizedAllowedValues(std::string_view text,
                                                     std::string& attributeName,
                                                     std::vector<std::string>& values)
    {
    ...
    static bool TryExtractConditionalPresence(const std::string& test, std::vector<std::string>& operands)
    {
    ...
    static bool TryExtractConditionalRequiredValue(const std::string& test, std::vector<std::string>& operands)
    {
        static const std::regex equality("^@(" + std::string(kAttributeNamePattern) + R"()\s*=\s*([^ ]+)$)");
        static const std::regex inequality("^@(" + std::string(kAttributeNamePattern) + R"()\s*!=\s*([^ ]+)$)");
    ...
    static bool TryExtractConditionalAllowedValues(const std::string& test, std::vector<std::string>& operands)
    {
        const auto separator = test.find(") or ");
        if (separator == std::string::npos || test.empty() || test.front() != '(')
        {
    ...
    static bool TryExtractConditionalForbiddenValues(const std::string& test, std::vector<std::string>& operands)
    {
    ...
    static bool TryExtractTriggerAllowedValues(const std::string& test, std::vector<std::string>& operands)
    {
    ...
    static SchematronPatternKind ClassifyAttributeBooleanExpression(const std::string& test, std::vector<std::string>& operands)
    {
        static const std::regex equality("^@(" + std::string(kAttributeNamePattern) + R"()\s*=\s*([^ ]+)$)");
        static const std::regex inequality("^@(" + std::string(kAttributeNamePattern) + R"()\s*!=\s*([^ ]+)$)");
        static const std::regex presence("^@(" + std::string(kAttributeNamePattern) + ")$");
                                      ...
                                      std::string sourceFile)
{
    ...
    static const std::regex relationshipType("^document\\(rels\\)//r:Relationship\\[@Id = current\\(\\)/@(" + std::string(SchematronHelper::kAttributeNamePattern) + R"()\]/@Type = '([^']+)'$)");
    static const std::regex relationshipExists("^document\\(rels\\)//r:Relationship\\[@Id = current\\(\\)/@(" + std::string(SchematronHelper::kAttributeNamePattern) + R"()\]$)");
    static const std::regex partReference("^Index-of\\(document\\('([^']+)'\\)//([^,]+),\\s*@(" + std::string(SchematronHelper::kAttributeNamePattern) + ")\\)$");
    static const std::regex partCountComparison("^@(" + std::string(SchematronHelper::kAttributeNamePattern) + ")\\s*([<>]=?)\\s*count\\(document\\('([^']+)'\\)//([^)]+)\\)\\s*\\+\\s*(-?[0-9]+)$");
    static const std::regex uniqueValues(
        R"(^count\(distinct-values\((.+)\)\)\s*=\s*count\((.+)\)$)");
    static const std::regex regexMatch(
        "^matches\\(@(" + std::string(SchematronHelper::kAttributeNamePattern) + "),\\s*\"([^\"]+)\"\\)$");
    static const std::regex stringLengthRange("^string-length\\(@(" + std::string(SchematronHelper::kAttributeNamePattern) + ")\\)\\s*>=\\s*([0-9]+)\\s+and\\s+string-length\\(@\\1\\)\\s*<=\\s*([0-9]+)$");
    static const std::regex stringLengthCompare("^string-length\\(@(" + std::string(SchematronHelper::kAttributeNamePattern) + ")\\)\\s*([<>]=?)\\s*([0-9]+)$");
    const std::string numericLiteral = R"(-?(?:0x[0-9A-Fa-f]+|[0-9]+(?:\.[0-9]+)?(?:[eE][+-]?[0-9]+)?f?))";
    const std::regex numericRange("^@(" + std::string(SchematronHelper::kAttributeNamePattern) + ")\\s*([<>]=?)\\s*(" + numericLiteral + ")\\s+and\\s+@\\1\\s*([<>]=?)\\s*(" + numericLiteral + ")$");
    const std::regex numericCompare("^@(" + std::string(SchematronHelper::kAttributeNamePattern) + ")\\s*([<>]=?)\\s*(" + numericLiteral + ")$");
    const std::regex numericAttributeCompare("^@(" + std::string(SchematronHelper::kAttributeNamePattern) + ")\\s*([<>]=?)\\s*@(" + std::string(SchematronHelper::kAttributeNamePattern) + ")$");

    if (SchematronHelper::Match(rule.test, relationshipType, match))
    {
        rule.kind = SchematronPatternKind::RelationshipType;
        rule.operands = SchematronHelper::Captures(match);
    }
    else if (SchematronHelper::Match(rule.test, relationshipExists, match))
    {
        rule.kind = SchematronPatternKind::RelationshipExists;
        rule.operands = SchematronHelper::Captures(match);
    }
    else if (SchematronHelper::Match(rule.test, partReference, match))
    {
        rule.kind = SchematronPatternKind::PartReferenceExists;
        rule.operands = SchematronHelper::Captures(match);
    }
    else if (SchematronHelper::Match(rule.test, partCountComparison, match))
    {
        rule.kind = SchematronPatternKind::PartCountComparison;
        rule.operands = SchematronHelper::Captures(match);
    }
    else if (SchematronHelper::Match(rule.test, uniqueValues, match))
    {
        rule.operands = SchematronHelper::Captures(match);
    ...
    else if (SchematronHelper::Match(rule.test, regexMatch, match))
    {
        rule.kind = SchematronPatternKind::AttributeRegex;
        rule.operands = SchematronHelper::Captures(match);
    }
    else if (SchematronHelper::Match(rule.test, stringLengthRange, match) || SchematronHelper::Match(rule.test, stringLengthCompare, match))
    {
        rule.kind = SchematronPatternKind::AttributeStringLength;
        rule.operands = SchematronHelper::Captures(match);
    }
    else if (SchematronHelper::Match(rule.test, numericRange, match))
    {
        rule.kind = SchematronPatternKind::AttributeNumericRange;
        rule.operands = SchematronHelper::Captures(match);
    }
    else if (SchematronHelper::Match(rule.test, numericCompare, match))
    {
        rule.kind = SchematronPatternKind::AttributeNumericComparison;
        rule.operands = SchematronHelper::Captures(match);
    }
    else if (SchematronHelper::Match(rule.test, numericAttributeCompare, match))
    {
        rule.kind = SchematronPatternKind::AttributeNumericAttributeComparison;
        rule.operands = SchematronHelper::Captures(match);
    }
    else if (SchematronHelper::IsAttributeName(rule.test))
    {
    ...
    {
        rule.kind = SchematronHelper::ClassifyAttributeBooleanExpression(rule.test, rule.operands);
    }
