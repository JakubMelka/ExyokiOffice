...
/// File-local tree builders and encoding helpers behind the JSON document model.
class DocumentModelJsonHelper
{
public:
    // --- Tolerant accessors -----------------------------------------------------
    //
    ...
    static std::string GetString(const Json& object, std::string_view key)
    {
    ...
    static bool GetBool(const Json& object, std::string_view key)
    {
        const auto it = object.find(key);
        if (it == object.end())
        {
    ...
    static Int64 GetInt(const Json& object, std::string_view key, Int64 defaultValue = 0)
    {
        const auto it = object.find(key);
        if (it == object.end())
        {
    ...
    static Real GetDouble(const Json& object, std::string_view key, Real defaultValue = 0.0)
    {
        const auto it = object.find(key);
        if (it == object.end())
        {
    ...
    static const Json* GetArray(const Json& object, std::string_view key)
    {
        const auto it = object.find(key);
        return it != object.end() && it->is_array() ? &*it : nullptr;
    }

    static const Json* GetObject(const Json& object, std::string_view key)
    {
        const auto it = object.find(key);
        return it != object.end() && it->is_object() ? &*it : nullptr;
    }

    static void SetIfNotEmpty(Json& object, std::string_view key, const std::string& value)
    {
        if (!value.empty())
        {
    ...
    static void SetIfTrue(Json& object, std::string_view key, bool value)
    {
        if (value)
        {
    ...
    static Json WordInlinesToJson(const std::vector<WordInline>& inlines)
    {
    ...
    static Json WordInlineToJson(const WordInline& node)
    {
    ...
    static Json WordBlockToJson(const WordBlock& block)
    {
        Json object = Json::object();
        switch (block.Kind)
        {
    ...
    static Json WordBlocksToJson(const std::vector<WordBlock>& blocks)
    {
        Json array = Json::array();
    ...
    static Json WordModelToJson(const WordDocumentModel& model)
    {
        Json object = Json::object();
    ...
    static Json ExcelModelToJson(const ExcelWorkbookModel& model)
    {
        Json object = Json::object();
    ...
    static std::string_view PptShapeTypeToken(PptShape::Type kind)
    {
        switch (kind)
        {
    ...
    static Json PptTextFrameToJson(const PptTextFrame& frame)
    {
    ...
    static Json PptShapeToJson(const PptShape& shape)
    {
        Json object = Json::object();
    ...
    static Json PowerPointModelToJson(const PowerPointDeckModel& model)
    {
        Json object = Json::object();
    ...
    static WordInline WordInlineFromJson(const Json& object, std::vector<ToolDiagnostic>& diagnostics)
    {
        WordInline node;
    ...
    static std::vector<WordInline> WordInlinesFromJson(const Json& array, std::vector<ToolDiagnostic>& diagnostics)
    {
    ...
    static WordBlock WordBlockFromJson(const Json& object, std::vector<ToolDiagnostic>& diagnostics)
    {
        WordBlock block;
        const auto type = GetString(object, "type");
        if (type == "table")
    ...
    static std::vector<WordBlock> WordBlocksFromJson(const Json& array, std::vector<ToolDiagnostic>& diagnostics)
    {
    ...
    static WordDocumentModel WordModelFromJson(const Json& object, std::vector<ToolDiagnostic>& diagnostics)
    {
    ...
    static ExcelWorkbookModel ExcelModelFromJson(const Json& object, std::vector<ToolDiagnostic>& diagnostics)
    {
    ...
    static PptTextFrame PptTextFrameFromJson(const Json& array)
    {
        PptTextFrame frame;
    ...
    static PptShape PptShapeFromJson(const Json& object, std::vector<ToolDiagnostic>& diagnostics)
    {
        PptShape shape;
        const auto type = GetString(object, "type");
        if (type == "placeholder")
    ...
    static PowerPointDeckModel PowerPointModelFromJson(const Json& object, std::vector<ToolDiagnostic>& diagnostics)
    {
...
{
    // Model JSON is also hand-written, so an unpadded final group is
    // accepted the way every other JSON tool accepts it.
    return Base64::Decode(text, out, Base64::Padding::Optional);
}
...
Json BuildModelTree(const DocumentModel& model, bool embedMedia)
{
    ...
    DocumentModelJsonHelper::SetIfNotEmpty(properties, "title", model.Properties.Title);
    DocumentModelJsonHelper::SetIfNotEmpty(properties, "subject", model.Properties.Subject);
    DocumentModelJsonHelper::SetIfNotEmpty(properties, "creator", model.Properties.Creator);
    DocumentModelJsonHelper::SetIfNotEmpty(properties, "keywords", model.Properties.Keywords);
    DocumentModelJsonHelper::SetIfNotEmpty(properties, "description", model.Properties.Description);
    DocumentModelJsonHelper::SetIfNotEmpty(properties, "lastModifiedBy", model.Properties.LastModifiedBy);
    DocumentModelJsonHelper::SetIfNotEmpty(properties, "category", model.Properties.Category);
    DocumentModelJsonHelper::SetIfNotEmpty(properties, "created", model.Properties.Created);
    DocumentModelJsonHelper::SetIfNotEmpty(properties, "modified", model.Properties.Modified);
    DocumentModelJsonHelper::SetIfNotEmpty(properties, "application", model.Properties.Application);
    DocumentModelJsonHelper::SetIfNotEmpty(properties, "company", model.Properties.Company);
    if (!properties.empty())
    {
    ...
    if (!model.Media.empty())
    {
        ...
        for (const auto& item : model.Media)
        {
            ...
            DocumentModelJsonHelper::SetIfNotEmpty(itemObject, "file", item.FileName);
            DocumentModelJsonHelper::SetIfNotEmpty(itemObject, "contentType", item.ContentType);
    ...
    switch (model.Family)
    {
        case DocumentFamily::Word:
            root["document"] = model.Word ? DocumentModelJsonHelper::WordModelToJson(*model.Word) : Json::object();
            break;
        case DocumentFamily::Excel:
            root["document"] = model.Excel ? DocumentModelJsonHelper::ExcelModelToJson(*model.Excel) : Json::object();
            break;
        case DocumentFamily::PowerPoint:
            root["document"] = model.PowerPoint ? DocumentModelJsonHelper::PowerPointModelToJson(*model.PowerPoint) : Json::object();
            break;
...
DocumentModel ParseModelTree(const Json& tree, std::vector<ToolDiagnostic>& diagnostics)
{
    ...
    const auto format = DocumentModelJsonHelper::GetString(tree, "format");
    if (!format.empty() && format != "exyokioffice-document")
    {
        DocumentModelJsonHelper::Warn(diagnostics, "Unexpected envelope format identifier", format);
    }
    const auto version = DocumentModelJsonHelper::GetInt(tree, "version", DocumentModelVersion);
    if (version > DocumentModelVersion)
    {
        DocumentModelJsonHelper::Warn(diagnostics, "Envelope version is newer than this tool; parsing best-effort",
                                      std::to_string(version));
    }

    const auto family = DocumentModelJsonHelper::GetString(tree, "family");
    if (family == "word")
    {
    ...
    if (const auto* properties = DocumentModelJsonHelper::GetObject(tree, "properties"))
    {
        model.Properties.Title = DocumentModelJsonHelper::GetString(*properties, "title");
        model.Properties.Subject = DocumentModelJsonHelper::GetString(*properties, "subject");
        model.Properties.Creator = DocumentModelJsonHelper::GetString(*properties, "creator");
        model.Properties.Keywords = DocumentModelJsonHelper::GetString(*properties, "keywords");
        model.Properties.Description = DocumentModelJsonHelper::GetString(*properties, "description");
        model.Properties.LastModifiedBy = DocumentModelJsonHelper::GetString(*properties, "lastModifiedBy");
        model.Properties.Category = DocumentModelJsonHelper::GetString(*properties, "category");
        model.Properties.Created = DocumentModelJsonHelper::GetString(*properties, "created");
        model.Properties.Modified = DocumentModelJsonHelper::GetString(*properties, "modified");
        model.Properties.Application = DocumentModelJsonHelper::GetString(*properties, "application");
        model.Properties.Company = DocumentModelJsonHelper::GetString(*properties, "company");
    }

    if (const auto* media = DocumentModelJsonHelper::GetArray(tree, "media"))
    {
        for (const auto& itemValue : *media)
        {
            ...
            MediaReference item;
            item.Id = DocumentModelJsonHelper::GetString(itemValue, "id");
            item.FileName = DocumentModelJsonHelper::GetString(itemValue, "file");
            item.ContentType = DocumentModelJsonHelper::GetString(itemValue, "contentType");
            const auto data = DocumentModelJsonHelper::GetString(itemValue, "data");
            if (!data.empty() && !DecodeBase64(data, item.Data))
            {
                DocumentModelJsonHelper::Warn(diagnostics, "Invalid base64 media data", item.Id);
                item.Data.clear();
            }
    ...
    const auto* document = DocumentModelJsonHelper::GetObject(tree, "document");
    if (document == nullptr)
    {
        DocumentModelJsonHelper::Warn(diagnostics, "Envelope has no document payload");
    ...
    {
        case DocumentFamily::Word:
            model.Word = DocumentModelJsonHelper::WordModelFromJson(*document, diagnostics);
            break;
        case DocumentFamily::Excel:
            model.Excel = DocumentModelJsonHelper::ExcelModelFromJson(*document, diagnostics);
            break;
        case DocumentFamily::PowerPoint:
            model.PowerPoint = DocumentModelJsonHelper::PowerPointModelFromJson(*document, diagnostics);
            break;
        case DocumentFamily::Unknown:
            break;
    }
