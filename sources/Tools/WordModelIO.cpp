...
void Info(std::vector<ToolDiagnostic>& diagnostics, std::string message, std::string context = {})
{
    diagnostics.push_back(ToolDiagnostic{ToolSeverity::Info, std::move(message), std::move(context)});
}

int HeadingLevelFromStyleId(const std::string& styleId)
{
...
std::string EnumToken(const std::optional<TEnum>& value)
{
    return value ? EnumValue<TEnum>(*value).ToString() : std::string();
}

template <typename TEnum>
std::optional<TEnum> TokenToEnum(const std::string& token)
{
    if (token.empty())
    {
        return std::nullopt;
                            ...
                            std::vector<ToolDiagnostic>& diagnostics)
{
    ...
    WordModelReader reader(editor, options, model, diagnostics);
    reader.Read();
    return model;
}
                    ...
                    std::vector<ToolDiagnostic>& diagnostics)
{
    ...
    WordModelWriter writer(model, diagnostics);
