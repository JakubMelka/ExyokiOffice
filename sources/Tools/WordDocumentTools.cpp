...
/// File-local helpers behind the Word document tools.
class WordDocumentToolsHelper
{
public:
    static constexpr std::string_view WordNamespace =
        "http://schemas.openxmlformats.org/wordprocessingml/2006/main";

    static void AddError(std::vector<ToolDiagnostic>& diagnostics, std::string message, const std::filesystem::path& path = {})
    {
        diagnostics.push_back({ToolSeverity::Error, std::move(message), path.string()});
    }

    static bool ValidateWordPackage(const std::filesystem::path& path, std::vector<ToolDiagnostic>& diagnostics)
    {
    ...
    static bool HasDescendant(const std::shared_ptr<OpenXMLElement>& root, std::string_view localName)
    {
        if (!root)
        {
            return false;
    ...
    static bool HasPageBreak(const std::shared_ptr<OpenXMLElement>& root)
    {
        if (!root)
        {
            return false;
    ...
    static std::vector<std::pair<Size, Size>> FindSegments(
    ...
    static std::filesystem::path PartPath(const std::filesystem::path& directory, std::string_view prefix,
                                  ...
                                  const WordSplitOptions& options)
{
    ...
    {
        WordDocumentToolsHelper::AddError(result.Diagnostics, "Paragraph count must be greater than zero");
        return result;
    }
    if (options.Strategy == WordSplitStrategy::Marker && options.Marker.empty())
    {
        WordDocumentToolsHelper::AddError(result.Diagnostics, "Split marker must not be empty");
        return result;
    }
    // WordDocumentToolsHelper::PartPath() concatenates the prefix into every output name, so a prefix
    ...
    {
        WordDocumentToolsHelper::AddError(result.Diagnostics, "Output prefix is not a plain file name", options.OutputPrefix);
        return result;
    }
    if (!WordDocumentToolsHelper::ValidateWordPackage(inputFile, result.Diagnostics))
    {
        return result;
    }
    ...
    if (!source)
    {
        WordDocumentToolsHelper::AddError(result.Diagnostics, "Failed to open Word document", inputFile);
        return result;
    }
    ...
    if (error)
    {
        WordDocumentToolsHelper::AddError(result.Diagnostics, "Failed to create output directory", outputDirectory);
        return result;
    }

    const auto sourceBytes = source->SaveToMemory();
    const auto blocks = source->BodyBlocks();
    const auto ranges = WordDocumentToolsHelper::FindSegments(blocks, options);
    if (ranges.empty())
    {
        WordDocumentToolsHelper::AddError(result.Diagnostics, "The source document has no body content", inputFile);
        return result;
    }
    ...
    {
        auto path = WordDocumentToolsHelper::PartPath(outputDirectory, options.OutputPrefix, part + 1, ranges.size());
        if (!options.Overwrite && std::filesystem::exists(path))
        {
            WordDocumentToolsHelper::AddError(result.Diagnostics, "Output file already exists", path);
            return result;
        }
    ...
    for (Size part = 0; part < ranges.size(); ++part)
    {
        ...
        if (!slice || !output)
        {
            WordDocumentToolsHelper::AddError(result.Diagnostics, "Failed to create split document", path);
            return result;
        }
        ...
        {
            WordDocumentToolsHelper::AddError(result.Diagnostics, "Failed to write split document", path);
            return result;
        }
                                   ...
                                   const WordMergeOptions& options)
{
    ...
    if (inputFiles.empty())
    {
        WordDocumentToolsHelper::AddError(result.Diagnostics, "At least one input document is required");
        return result;
    }
    if (!options.Overwrite && std::filesystem::exists(outputFile))
    {
        WordDocumentToolsHelper::AddError(result.Diagnostics, "Output file already exists", outputFile);
        return result;
    }
    auto output = WordDocumentEditor::CreateNew();
    if (!output)
    {
        WordDocumentToolsHelper::AddError(result.Diagnostics, "Failed to create output document", outputFile);
        return result;
    }
    ...
    {
        if (!WordDocumentToolsHelper::ValidateWordPackage(input, result.Diagnostics))
        {
            return result;
        }
        ...
        if (!source)
        {
            WordDocumentToolsHelper::AddError(result.Diagnostics, "Failed to open Word document", input);
            return result;
        }
        ...
        {
            WordDocumentToolsHelper::AddError(result.Diagnostics, "Failed to merge Word document", input);
            return result;
        }
    ...
    {
        WordDocumentToolsHelper::AddError(result.Diagnostics, "Failed to write merged document", outputFile);
        return result;
    }
    result.Ok = true;
    return result;
