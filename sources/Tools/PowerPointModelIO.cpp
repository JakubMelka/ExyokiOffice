...
/// File-local helpers for the presentation document model round trip.
class PowerPointModelIoHelper
{
public:
    ...
    static constexpr PlaceholderTokenEntry kPlaceholderTokens[] = {
    ...
    static std::string PlaceholderToken(PresentationPlaceholderValues::Value value)
    {
    ...
    static bool IsTitleToken(const std::string& token)
    {
    ...
    static std::string ShapeName(const P::PresentationShape& shape)
    {
    ...
    static Int64 ToEmu(const MeasuringUnits& value)
    {
        return static_cast<Int64>(std::llround(value.ToEmu().GetValue()));
    }

    static PptTransform TransformToModel(const P::PresentationShapeTransform& transform)
    {
        PptTransform model;
        model.Present = true;
    ...
    static P::PresentationShapeTransform TransformFromModel(const PptTransform& model)
    {
    ...
    static P::PresentationShapeTransform DefaultTransform(Int64 xEmu, Int64 yEmu, Int64 cxEmu,
                                                          Int64 cyEmu)
    {
        P::PresentationShapeTransform transform;
    ...
    static PptTextFrame FrameToModel(const P::PresentationTextFrame& frame)
    {
        PptTextFrame model;
    ...
    static P::PresentationTextFrame FrameFromModel(const PptTextFrame& model)
    {
    ...
    static std::string FramePlainText(const PptTextFrame& frame)
    {
        std::string text;
        for (const auto& paragraph : frame.Paragraphs)
        {
    ...
    static PptTableModel TableToModel(const P::PresentationTableData& table)
    {
        PptTableModel model;
    ...
    static P::PresentationTableData TableFromModel(const PptTableModel& model,
                                  ...
                                  std::vector<ToolDiagnostic>& diagnostics)
{
    ...
    PowerPointModelIoHelper::PowerPointModelReader reader(editor, options, model, diagnostics);
    reader.Read();
    return model;
}
                          ...
                          std::vector<ToolDiagnostic>& diagnostics)
{
    ...
    PowerPointModelIoHelper::PowerPointModelWriter writer(model, diagnostics);
