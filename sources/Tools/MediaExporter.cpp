...
bool IsMediaContentType(std::string_view contentType)
{
    return contentType.starts_with("image/") || contentType.starts_with("audio/") ||
           contentType.starts_with("video/");
}

bool IsEmbeddedObjectPart(std::string_view contentType, std::string_view uri)
{
...
const std::unordered_map<std::string, std::string>& ContentTypeExtensionTable()
{
...
std::string ExtensionFromUri(std::string_view uri)
{
...
std::string BaseNameFromUri(std::string_view uri)
{
    const auto slash = uri.rfind('/');
    const auto fileName = slash == std::string_view::npos ? uri : uri.substr(slash + 1);
    const auto dot = fileName.rfind('.');
...
std::string ResolveExtension(std::string_view uri, std::string_view contentType, const std::vector<Byte>& data)
{
...
std::filesystem::path MakeUniquePath(const std::filesystem::path& outDir, const std::string& baseName,
...
MediaExportResult ExportMedia(const OpenXmlPackage& package, const std::filesystem::path& outDir, bool overwrite)
{
    ...
    for (const auto& part : CollectAllParts(package))
    {
        ...
        if (!part->IsBinaryPart() || !(IsMediaContentType(contentType) || IsEmbeddedObjectPart(contentType, uri)))
        {
            continue;
        }
        ...
        const auto rawExtension = ResolveExtension(uri, contentType, data);
        const auto rawBaseName = BaseNameFromUri(uri);
        ...
        const auto outputPath = MakeUniquePath(outDir, baseName, extension, usedNames);
