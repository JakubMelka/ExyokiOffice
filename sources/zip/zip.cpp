...
struct zip_t *zip_openwitherror(const char *zipname, int level, char mode,
  ...
  *errnum = 0;
  ...
  case 'w':
    ...
    if (!mz_zip_writer_init_file_v2(&(zip->archive), zipname, 0, 0)) {
...
int zip_entry_close(struct zip_t *zip) {
  ...
  needs_zip64 =
      ...
      (pzip && pzip->m_pState && pzip->m_pState->m_zip64);
  ...
  if (needs_zip64) {
    ...
    MZ_WRITE_LE32(local_dir_footer + 8,
                  (mz_uint32)MZ_MIN(zip->entry.comp_size, MZ_UINT32_MAX));
    MZ_WRITE_LE32(local_dir_footer + 12,
                  (mz_uint32)MZ_MIN(zip->entry.uncomp_size, MZ_UINT32_MAX));
  }
...
struct zip_t *zip_cstream_openwitherror(FILE *stream, int level, char mode,
  ...
  *errnum = 0;
  ...
  case 'w':
    // Create a new archive.
    if (!mz_zip_writer_init_cfile(&(zip->archive), stream, 0)) {
      // Cannot initialize zip_archive writer
      *errnum = ZIP_EWINIT;
      goto cleanup;
    }
