...
std::string_view CachedKindToken(Excel::FormulaCachedValueKind kind)
{
    switch (kind)
    {
...
Excel::FormulaCachedValueKind CachedKindFromToken(const std::string& token)
{
    if (token == "string")
    {
...
bool ParseNumber(const std::string& text, Real& value)
{
    if (text.empty())
    {
        return false;
...
void ReadSheet(const Excel::Worksheet::Ptr& worksheet, const Excel::SharedStringTableService& sharedStrings,
...
void WriteSheet(const Excel::Worksheet::Ptr& worksheet, const ExcelSheetModel& sheet,
                             ...
                             std::vector<ToolDiagnostic>& diagnostics)
{
    ...
    for (const auto& worksheet : editor.Worksheets())
    {
        ...
        ReadSheet(worksheet, sharedStrings, sheet, diagnostics);
                     ...
                     std::vector<ToolDiagnostic>& diagnostics)
{
    ...
    for (const auto& sheet : model.Excel->Sheets)
    {
        ...
        if (!worksheet)
        {
            Warn(diagnostics, "Worksheet could not be created", sheetName);
            continue;
        }
        WriteSheet(worksheet, sheet, diagnostics);
    }
