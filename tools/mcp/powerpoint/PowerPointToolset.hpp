...
/** @brief A presentation held open by a session. */
class PowerPointDocumentHandle final : public DocumentHandle
{
public:
    /// @param limits Applied when LoadFromMemory reopens the package (undo, session bridge).
    PowerPointDocumentHandle(PowerPoint::PowerPointDocumentEditor::Ptr editor, OpenXmlPackageLimits limits);
    ...
    /// The editor the PowerPoint toolset drives; never null for a live handle.
    [[nodiscard]] PowerPoint::PowerPointDocumentEditor& Editor() const noexcept { return *m_editor; }

private:
    PowerPoint::PowerPointDocumentEditor::Ptr m_editor;
    OpenXmlPackageLimits m_packageLimits;
};

/** @brief PowerPoint implementation of the family operations the shared core needs. */
class PowerPointFamilyAdapter final : public FamilyAdapter
{
public:
    [[nodiscard]] Tools::DocumentFamily Family() const override;
...
/// Registers the PowerPoint slide, content, media, and design groups.
