...
std::string EscapeFormatLiteral(std::string_view text)
{
    std::string escaped;
    escaped.reserve(text.size());
    for (const char c : text)
...
std::vector<LocatedMatch> FindMatches(const std::string& text, std::string_view needle,
...
std::string MakeContext(const std::string& text, Size offset, Size length, Size contextChars)
{
...
std::string ApplyMatches(const std::string& text, const std::vector<LocatedMatch>& matches)
{
    std::string output;
...
DocumentFamily DetectFamily(const std::filesystem::path& path, std::vector<ToolDiagnostic>& diagnostics)
{
...
std::string CellText(const Excel::ExcelCellValue& value, const Excel::SharedStringTableService& sharedStrings)
{
...
[[nodiscard]] bool IsTextCell(const Excel::ExcelCellValue& value)
{
...
DocumentSearchResult SearchWordFamily(Word::WordDocumentEditor& editor, std::string_view needle,
...
DocumentSearchResult SearchExcelFamily(Excel::ExcelDocumentEditor& editor, std::string_view needle, Size contextChars,
...
DocumentSearchResult SearchPowerPointFamily(PowerPoint::PowerPointDocumentEditor& editor, std::string_view needle, Size contextChars,
                                            bool useRegex, bool ignoreCase)
{
    DocumentSearchResult result;
...
DocumentReplaceResult ReplaceWordFamily(Word::WordDocumentEditor& editor, std::string_view needle,
...
DocumentReplaceResult ReplaceExcelFamily(Excel::ExcelDocumentEditor& editor, std::string_view needle,
                                         std::string_view replacement, bool dryRun, bool useRegex,
                                         bool ignoreCase)
{
    DocumentReplaceResult result;
...
DocumentReplaceResult ReplacePowerPointFamily(PowerPoint::PowerPointDocumentEditor& editor, std::string_view needle,
                                              std::string_view replacement, bool dryRun, bool useRegex,
                                              bool ignoreCase)
{
    DocumentReplaceResult result;
...
TResult FailedToOpen(DocumentFamily family, const char* message, const std::filesystem::path& path)
{
    TResult result;
...
template <typename TResult>
TResult EmptyNeedle(DocumentFamily family)
{
    TResult result;
    result.Family = family;
...
DocumentReplaceResult SaveReplaced(DocumentReplaceResult result, OpenXmlPackage* package, bool dryRun,
                                        ...
                                        Size contextChars, bool useRegex, bool ignoreCase)
{
    ...
    result.Family = DetectFamily(path, result.Diagnostics);
    switch (result.Family)
    {
        case DocumentFamily::Word:
        {
            ...
            return editor ? SearchDocumentText(*editor, needle, contextChars, useRegex, ignoreCase)
                          : FailedToOpen<DocumentSearchResult>(DocumentFamily::Word,
        ...
        case DocumentFamily::Excel:
        {
            ...
            return editor ? SearchDocumentText(*editor, needle, contextChars, useRegex, ignoreCase)
                          : FailedToOpen<DocumentSearchResult>(DocumentFamily::Excel,
        ...
        case DocumentFamily::PowerPoint:
        {
            ...
            return editor ? SearchDocumentText(*editor, needle, contextChars, useRegex, ignoreCase)
                          : FailedToOpen<DocumentSearchResult>(DocumentFamily::PowerPoint,
...
DocumentSearchResult SearchDocumentText(Word::WordDocumentEditor& editor, std::string_view needle,
                                        Size contextChars, bool useRegex, bool ignoreCase)
{
    return needle.empty() ? EmptyNeedle<DocumentSearchResult>(DocumentFamily::Word)
                          : SearchWordFamily(editor, needle, contextChars, useRegex, ignoreCase);
}

DocumentSearchResult SearchDocumentText(Excel::ExcelDocumentEditor& editor, std::string_view needle,
                                        Size contextChars, bool useRegex, bool ignoreCase)
{
    return needle.empty() ? EmptyNeedle<DocumentSearchResult>(DocumentFamily::Excel)
                          : SearchExcelFamily(editor, needle, contextChars, useRegex, ignoreCase);
}

DocumentSearchResult SearchDocumentText(PowerPoint::PowerPointDocumentEditor& editor, std::string_view needle,
                                        Size contextChars, bool useRegex, bool ignoreCase)
{
    return needle.empty() ? EmptyNeedle<DocumentSearchResult>(DocumentFamily::PowerPoint)
                          : SearchPowerPointFamily(editor, needle, contextChars, useRegex, ignoreCase);
}
                                          ...
                                          bool ignoreCase)
{
    ...
    switch (result.Family)
    {
        case DocumentFamily::Word:
        {
            ...
            if (!editor)
            {
                return FailedToOpen<DocumentReplaceResult>(DocumentFamily::Word, "Failed to open Word document",
                                                           path);
            }
            return SaveReplaced(ReplaceDocumentText(*editor, needle, replacement, dryRun, useRegex, ignoreCase),
        ...
        case DocumentFamily::Excel:
        {
            ...
            if (!editor)
            {
                return FailedToOpen<DocumentReplaceResult>(DocumentFamily::Excel, "Failed to open Excel document",
                                                           path);
            }
            return SaveReplaced(ReplaceDocumentText(*editor, needle, replacement, dryRun, useRegex, ignoreCase),
        ...
        case DocumentFamily::PowerPoint:
        {
            ...
            if (!editor)
            {
                return FailedToOpen<DocumentReplaceResult>(DocumentFamily::PowerPoint,
                                                           "Failed to open PowerPoint document", path);
            }
            return SaveReplaced(ReplaceDocumentText(*editor, needle, replacement, dryRun, useRegex, ignoreCase),
...
DocumentReplaceResult ReplaceDocumentText(Word::WordDocumentEditor& editor, std::string_view needle,
                                          ...
                                          bool ignoreCase)
{
    return needle.empty() ? EmptyNeedle<DocumentReplaceResult>(DocumentFamily::Word)
                          : ReplaceWordFamily(editor, needle, replacement, dryRun, useRegex, ignoreCase);
}
...
DocumentReplaceResult ReplaceDocumentText(Excel::ExcelDocumentEditor& editor, std::string_view needle,
                                          ...
                                          bool ignoreCase)
{
    return needle.empty() ? EmptyNeedle<DocumentReplaceResult>(DocumentFamily::Excel)
                          : ReplaceExcelFamily(editor, needle, replacement, dryRun, useRegex, ignoreCase);
}
...
DocumentReplaceResult ReplaceDocumentText(PowerPoint::PowerPointDocumentEditor& editor, std::string_view needle,
                                          ...
                                          bool ignoreCase)
{
    return needle.empty() ? EmptyNeedle<DocumentReplaceResult>(DocumentFamily::PowerPoint)
                          : ReplacePowerPointFamily(editor, needle, replacement, dryRun, useRegex, ignoreCase);
}
